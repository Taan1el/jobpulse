...
JobPulse is a job search planner. It tracks which skills keep showing up in the listings you collect, rates how well those skills cover each target role, and turns the gaps into a queue of projects to build. Everything runs in the browser and is saved to `localStorage`.
...
- **Target listings**: sample listings with their requirements, a fit rating, and a note on what is missing.
- **Build queue**: tasks move from Next to In progress to Done (and can be reopened), and the header counts closed tasks.
...
- **Connection states**: Ready, Loading, Empty, and Error views for a job-board source. These are sample states; the app makes no network requests.
...
```text
shared/jobpulse.ts       Domain types and sample data
...
- The listings, signals, and tasks the app starts with are sample data, and the company names are fictional.
...
- Let users add and edit target listings instead of starting from the sample set.
